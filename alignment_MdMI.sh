...
#再与PriREN中苹果小RNA比
#owtie-build ../mdmiRNAPmiREN/miRNA_stem-loop_with_20bp_flanking_seq.fas mdPriREN/MdPriREN  创建bowtie索引
...
for i in `ls *fq*`
do
    nohup bowtie -q -p 20 -n 0 -m 1 --best --strata --un ${3}/${i%%.*}.noMdMI.fq -x ${index_dir}/${index_base} $i -S ${3}/${i%%.*}.sam 1>${3}/${i%%.*}.log 2>&1 &
done
